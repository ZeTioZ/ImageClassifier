...
const props = defineProps(['parent', 'imgSup', 'image']);
    ...
    <div class="relative">
      ...
      <div class="fixed bg-white rounded-lg shadow items-center justify-center inset-20 min-h-[250px]">
        <nav class="w-full bg-ls-vert-base">
          <h2 class="text-xl font-bold pt-2 pb-2">{{ image.filename }}</h2>
          ...
          <div class="w-1/3 min-h-[206px]">
            ...
            <div class="mt-0.5 overflow-y-auto scrollbar-hide">
              <div class="flex m-3">
                ...
                <Tag v-for="tag in image.tags" @delete="tagToDel()" :key="tag.tagname" :tagName="tag.tagname" :class="tag.cssColorClass" :inModal="'pomelo'"/>
              </div>
